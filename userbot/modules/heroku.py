...
@bot.on(geezbot_cmd(outgoing=True, pattern=r'set var (\w*) ([\s\S]*)'))
...
@bot.on(geezbot_cmd(outgoing=True, pattern="usage$"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"logs"))
async def _(dyno):
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
...
CMD_HELP.update({"heroku": f">`{geez}usage`"
                 "\nUsage: Check your heroku dyno hours remaining"
                 f"\n\n>`{geez}set var <NEW VAR> <VALUE>`"
                 "\nUsage: add new variable or update existing value variable"
                 "\n!!! WARNING !!!, after setting a variable the bot will restarted"
                 f"\n\n>`{geez}get var or .get var <VAR>`"
