        ...
        if not apprv and event.text != UNAPPROVED_MSG:
            if event.chat_id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    await event.reply(f"{DEF_UNAPPROVED_MSG}")
            else:
                await event.reply(f"{DEF_UNAPPROVED_MSG}")
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"notifoff$"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"notifon$"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"(?:approve|ok)\s?(.)?"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"(?:disapprove|nopm)\s?(.)?"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"block$"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"unblock$"))
...
@bot.on(geezbot_cmd(outgoing=True, pattern=r"(set|get|reset) pm_msg(?: |$)(\w*)"))
...
CMD_HELP.update(
    {
        "pm": f">`{geez}approve | {geez}ok`"
        "\nUsage: Approves the mentioned/replied person to PM."
        f"\n\n>`{geez}disapprove | .nopm`"
